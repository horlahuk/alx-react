...
describe("Notification tests", () => {
	...
	it("renders correct list items", () => {
		...
		wrapper.find("ul").forEach((node) => {
			expect(node.equals(<NotificationItem />)).toBe(true);
		});
		...
		expect(wrapper.find("ul").childAt(2).html()).toEqual(`<li data-urgent="true">${getLatestNotification()}</li>`); // Remove unnecessary escape characters
	});
	...
	it("renders an unordered list", () => {
		...
		expect(wrapper.find("ul").children()).toHaveLength(3);
		wrapper.find("ul").forEach((node) => {
			expect(node.equals(<NotificationItem />)).toBe(true); // Add the NotificationItem component to the test
		});
	});
